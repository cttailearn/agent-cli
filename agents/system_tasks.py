        ...
        payload={
            "prompt": "\n".join(
                [
                    ...
                    "3) 如果发现 core 记忆（soul/traits/identity/user）存在重复、冲突或过时内容：委派执行者进行精炼重写（合并去重、删过时、优化表述）。",
                    "4) 输出以要点形式给出。",
                ]
            )
        },
    ...
    AgentSystemTaskSpec(
        ...
        payload={
            "prompt": "\n".join(
                [
                    "系统已进入新的一天。",
                    "请回顾昨天对未来有用的稳定事实/偏好（不要泛泛总结）；如需要写入长期记忆（core/user），再委派执行者写入；如发现 core 记忆冗余或过时，也委派执行者精炼重写。",
                ]
            )
        },
    ),
